...
function inferGstType(placeOfSupply, companyState) {
  if (!placeOfSupply || !companyState) return 'intra';
  return normalizeState(placeOfSupply) === normalizeState(companyState) ? 'intra' : 'inter';
}

function splitGstInclusive(inclusiveAmount, gstRate) {
...
function splitGstTaxTotal(taxTotal, gstType = 'intra') {
...
function computeInvoiceGstFromItems(items = [], gstType = 'intra') {
  let subTotal = 0;
  let taxTotal = 0;
